...
EMCC := emcc
EMFLAGS := -o $(BUILD_DIR)/$(PROGRAM_NAME).html $(CFLAGS) -Os ./web/libraylib.a --shell-file ./web/minshell.html --preload-file assets -s USE_GLFW=3 -DPLATFORM_WEB
...
buildDesktop: $(OBJS)
	$(CC) $(CFLAGS) $(OBJS) $(LDFLAGS) -o $(BUILD_DIR)/$(PROGRAM_NAME)
buildWeb: $(C_SRCS)
	$(EMCC) $(C_SRCS) $(EMFLAGS)
